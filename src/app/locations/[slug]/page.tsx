...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title: `Glulam Manufacturer Insurance — ${loc.name}`,
    description: `${loc.blurb} Products liability, workers' comp, commercial property, and inland marine for ${loc.name} glulam manufacturers. 15-minute quotes.`,
    alternates: { canonical: url },
    openGraph: {
      title: `Glulam Manufacturer Insurance — ${loc.name} | Contractors Choice Agency`,
...
export default async function LocationPage({ params }: Props) {
  ...
  const serviceSchema = {
    ...
    name: `Glulam Manufacturer Insurance — ${loc.name}`,
    description: loc.blurb,
    url,
  ...
  return (
    <>
      ...
      <main>
            ...
            <div className="grid lg:grid-cols-12 gap-10 items-center">
              ...
              <FadeIn direction="left" className="lg:col-span-5">
                <div className="rounded-t-[10rem] rounded-b-3xl overflow-hidden border-4 border-white shadow-warm-lg">
                  <img
                    src="/images/lumber-yard.jpg"
                    alt={`Glulam manufacturing facility and lumber yard in ${loc.name}`}
        ...
        <section className="bg-cream py-16 md:py-20">
          <div className="container-tight">
            <FadeIn className="max-w-2xl mb-10">
              <span className="eyebrow"><span className="h-px w-8 bg-clay" />Coverage for {loc.name} manufacturers</span>
              <h2 className="mt-3 h-section">
                The full program, built for {loc.name} glulam manufacturers.
              </h2>
              <p className="mt-4 lead">
                From a single-plant operation to a multi-facility production group,
                we coordinate every line a {loc.name} glulam manufacturer needs.
              </p>
            </FadeIn>
        ...
        <CTABand
          title={`Insuring glulam manufacturers in ${loc.name} since 2005`}
